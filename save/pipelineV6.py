                ...
                if os.path.exists(psname):
                    ...
                    if ( os.path.exists(candfile) and os.path.exists(datfile) ):
                        cmd = "prepfold -noxwin -accelcand %s -accelfile %s -o %s %s"\
                              %(namesplit[1], candfile, outname, datfile)
...
def run_accelsearch(work_dir, basename):
    ...
    # Run accelsearch on each .dat file if we have not already done so
    print "Running accelsearch on %d %s files...\n" %(len(datfiles), suffix)
    pool_accel = mp.Pool(processes=ncores)
    for datfile in datfiles:
...
def run_singlepulse_search(work_dir, w_max=params.max_width, \
...
# Get and format the base name and results directory
fitsname = params.fitsname
