...
import sys
import time
import remote_scan_params as params
import sifting
import re
import shlex
import psr_utils
...
def check_dependencies(work_dir, fits_dir, fitsbase):
    ...
    if len(fl):
        ...
        print "  No %s*.fits files found in %s !" %(fitsbase, fits_dir)
...
def setup(basename, local_fits, local_results, host_fits, host_results):
    ...
    sub_dirs = ["rfi_products", "dedisperse"]
    ...
    if resume:
        ...
        dm_files = glob("%s/dedisperse/*" %(host_results))
        if len(dm_files):
            for dm_file in dm_files:
                shutil.move(dm_file, host_results)
            shutil.rmtree("%s/dedisperse" %(host_results))
...
if __name__ == "__main__":
    ...
    top_HOST    = "%s/scan%02d" %(sys.argv[1], scan_num)
    fits_HOST   = "%s/psrfits" %(top_HOST)
    results_HOST = "%s/search" %(top_HOST)

    # Relevant directories on local space
    fits_LOCAL    = params.fits_dir
    top_LOCAL = params.results_dir
    results_LOCAL = "%s/scan%02d" %(top_LOCAL, scan_num)
   
    # basename for fits files
    basename = "scan%02d_%s" %(scan_num, params.basename)
